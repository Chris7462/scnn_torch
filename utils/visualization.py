...
def add_exist_text(img, exist_pred, threshold=0.5):
    """
    Add existence prediction text to image.
    ...
    """
    img = img.copy()
    exist_binary = [1 if exist_pred[i] > threshold else 0 for i in range(4)]
    text = f"{exist_binary}"
    cv2.putText(img, text, (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 255, 255), 2)
    return img
...
def prepare_visualization_batch(imgs, seg_preds, exist_preds, transform_img=None, threshold=0.5):
    ...
    for i in range(len(seg_preds)):
        ...
        lane_img = add_exist_text(lane_img, exist_pred, threshold)
